...
``` bash
curl -s https://raw.githubusercontent.com/huntergps/odoo-15-docker-compose/master/run.sh | sudo bash -s odoo-one 10015 20015
```

to set up first Odoo instance @ `localhost:10015` (default master password: `admin15@2021`)

and

``` bash
curl -s https://raw.githubusercontent.com/huntergps/odoo-15-docker-compose/master/run.sh | sudo bash -s odoo-two 11015 21015
```

to set up another Odoo instance @ `localhost:11015` (default master password: `admin15@2021`)
...
``` sh
$ git clone https://github.com/huntergps/odoo-15-docker-compose
...
* Default database password (**admin_passwd**) is `admin15@2021`, please change it @ [etc/odoo.conf#L60](/etc/odoo.conf#L60)
